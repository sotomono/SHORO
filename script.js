...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadKanjiBtn = document.getElementById('loadKanjiBtn');
    const shareBtn = document.getElementById('shareBtn');

    // URLパラメータ確認
  const params = new URLSearchParams(window.location.search);

  const sharedKanji = params.get('kanji');

   if (sharedKanji) {
      kanjiInput.value = sharedKanji;

      // 自動読み込み
      loadKanji(sharedKanji, true);
    }
   ...
   async function loadKanji(kanji, autoPlay = false) {
    ...
    try {
        ...
        const url =
            `https://cdn.jsdelivr.net/gh/KanjiVG/kanjivg/kanji/${codePoint}.svg`;
